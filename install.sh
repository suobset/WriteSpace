...
# Remove old Python TUI launcher if it exists to avoid conflict
if [ -f /usr/local/bin/writespace-tui ]; then
    echo "Removing old Python TUI launcher (/usr/local/bin/writespace-tui)..."
    sudo rm -f /usr/local/bin/writespace-tui
fi

echo "Building and Installing WriteSpace TUI (C++)..."
if [ -d "build_cpp_tui" ]; then
    echo "Removing previous C++ TUI build directory..."
    rm -rf build_cpp_tui
fi
mkdir build_cpp_tui
cd build_cpp_tui
